...
function submit($jobid, $email){
	...
	$dama = $_SESSION['dama'];
	echo 'DAMA: '.$dama.' <br>';
	$args = escapeshellarg('-i '.$appurl.'/MyCLADE/jobs/'.$jobid.'/data.fa')." ".escapeshellarg('-N '.$jobid)." ".escapeshellarg('-e '.$e_value)." ".escapeshellarg('-W http://'.$appurl.'/MetaCLADE_webserver/MyCLADE/jobs/'.$jobid)." ".escapeshellarg('--sge ')." ".escapeshellarg('--pe smp ')." ".escapeshellarg('-j '.$nb_jobs)." ";
	if($dama == 'true'){
