<?php include("./includes/header.php"); ?>
	...
	<h2> Annotation with all Pfam domains</h2>
		...
		<form name="large_annotation_form" method = "post" action="submit.php?form=large"  enctype="multipart/form-data" onsubmit="return large_form_submission()">
			...
			<div id='submission'>
			<br/>
			<input class='btn' type="submit" value="Search" name = "submit"/>&emsp;<input class='btn' type="reset" value="Reset" onclick="reset_btn('large')"/>&emsp;
			<span class='tooltip'><input class='btn' type="button" value="Example" onclick="fill_exemple_form('large')"/><span class='tooltiptext'>It uploads a precalculated example, you can choose whether or not to use DAMA. All other parameters are defined with default values.</span></span>
			</div>			
		</form>	
	</section>
